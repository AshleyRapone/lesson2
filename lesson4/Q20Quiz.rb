...
# One of the requirements of the project is to change the implementation from using basic looping structures
# to using iterative methods instead.
...
# The team comes up with four options to replace the legacy code, which you then discuss during a code review.
# Select all the options which replicate the functionality of the legacy code.
...
=begin
Iterates through the array and for each ahsh assign to lead
  -for the name for the lead split it
  -for names variable iterate and capitalize the first and last name
  -then join the name back together
Iterates through the array and for each hash assign it to lead
  -if days since login value is less than 60 and mailing_list is true assign to usable_leads
=end
...
=begin
You need to split lead[:name] before you capitalize or else you only capitalize the first name
=end
...
=begin
We cant have <= we need < because the old code had < we need to maintain what the old code has
=end
...
=begin
reject eliminates leads where neither of those things are true rather than just one of them.
